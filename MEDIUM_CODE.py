...
def evaluate_students(student_answers, answer_key):
    ...
    for _, student_row in student_answers.iterrows():
        ...
        for question in questions:
            ...
            # === 1. Base scoring using sentence semantic similarity ===
            base_similarity = sentence_flow_score(student_answer, key_answer)
            base_mark = (base_similarity / 100) * max_mark

            # === 2. Plagiarism detection using n-gram similarity ===
            ...
            raw_plag_score = np.mean(plagiarism_scores) if plagiarism_scores else 0
            plagiarism_pct = max(60, 100 - raw_plag_score)  # soften penalty: at least 60%

            # === 3. Flow as a bonus ===
            flow_scores = [sentence_flow_score(student_answer, ans) for ans in other_answers]
            flow_pct = np.mean(flow_scores) if flow_scores else 50  # neutral default
            bonus = (flow_pct - 50) / 100  # anything above 50 is bonus

            # === 4. Final score computation ===
            adjusted_mark = base_mark * (plagiarism_pct / 100)
            bonus_mark = base_mark * bonus
            final_mark = adjusted_mark + bonus_mark
            final_mark = round(min(final_mark, max_mark), 2)  # clamp to max mark
            total_marks += final_mark

            # === 5. Store details ===
            student_result[f'{question} - Base %'] = round(base_similarity, 2)
